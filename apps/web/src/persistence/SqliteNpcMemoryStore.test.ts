...
describe('SqliteNpcMemoryStore — dedupe (Slice C3)', () => {
  ...
  it('persists dedupe_key in its own column', async () => {
    ...
    try {
      ...
      const row = db.prepare('SELECT dedupe_key FROM npc_memories WHERE memory_id = ?').get('m1')
  ...
  it('importance and entitySnapshots survive a write -> read round trip', async () => {
    ...
    try {
      ...
      const got = await store.listForNpc({ worldId: 'world-1', sessionId: 'session-1', npcId: 'npc-1' })
      expect(got[0]?.importance).toBe(4)
    ...
    try {
      ...
      db.prepare(
        `INSERT INTO npc_memories
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).run('broken', 'world-1', 'session-1', 'npc-1', 'npc_belief', 1, 1, '{ not valid json', '2026-06-23T10:00:00.000Z', 'evt-1')
