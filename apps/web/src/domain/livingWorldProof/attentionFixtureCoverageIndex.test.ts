...
const fixtureCoverageIndex: readonly FixtureMapping[] = [
  ...
  ...mapIds(['R7'], 'attentionAbsenceWitnessProvenance.test.ts', 'R7 refuses a structurally non-closed relation through the certified-absence accessor'),
  ...mapIds(['R8'], 'attentionAbsenceWitnessProvenance.test.ts', 'R8 refuses an interval that is not fully covered by the accessor horizon'),
  ...mapIds(['R9'], 'attentionAbsenceWitnessProvenance.test.ts', 'R9 refuses a forged or malformed accessor certificate through the absence builder'),
  ...mapIds(['R10'], 'attentionAbsenceWitnessProvenance.test.ts', 'R10 refuses a textual non-certificate without treating a failed text search as absence'),
