...
const DERIVATION_VARIATIONS: readonly (readonly [
  ...
  ['communicationAuthorityAccessorContractVersion', { communicationAuthorityAccessorContractVersion: 'fixture-communication-authority-accessor-v2' }],
...
describe('B4 / RN019 §9.3 — the derivation bundle is the closed thirteen-field set', () => {
  it('declares exactly the thirteen fields, in RN019\'s declared order', () => {
    ...
    expect(ATTENTION_CANDIDATE_DERIVATION_DEPENDENCY_FIELDS).toHaveLength(21)
  })
  ...
  it('builds a default bundle from the pinned constants, with no field left ambient', () => {
    expect(derivation()).toEqual({
      ...
      communicationAuthorityAccessorContractVersion: ATTENTION_COMMUNICATION_AUTHORITY_ACCESSOR_VERSION,
    })
  ...
  it('bumps the derivation and ranking key schemas to their explicit versioned strings', () => {
    expect(ATTENTION_CANDIDATE_DERIVATION_CACHE_KEY_SCHEMA_VERSION)
      .toBe('attention-candidate-derivation-cache-key-v5')
